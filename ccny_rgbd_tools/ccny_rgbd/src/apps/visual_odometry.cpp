  ...
  frame_count_(0)
{
  ...
  odom_publisher_ = nh_.advertise<OdomMsg>(
    "vo", queue_size_);
  pose_stamped_publisher_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>(
    "pose", queue_size_);
...
void VisualOdometry::publishPoseStamped(const std_msgs::Header& header)
{
  ...
  geometry_msgs::PoseWithCovarianceStamped::Ptr pose_stamped_cov_msg;
  pose_stamped_msg = boost::make_shared<geometry_msgs::PoseStamped>();
  pose_stamped_cov_msg = boost::make_shared<geometry_msgs::PoseWithCovarianceStamped>();

  pose_stamped_cov_msg->header.stamp    = header.stamp;
  pose_stamped_cov_msg->header.frame_id = fixed_frame_;      
  tf::poseTFToMsg(f2b_, pose_stamped_msg->pose);
  pose_stamped_cov_msg->pose.pose = pose_stamped_msg->pose;
  pose_stamped_publisher_.publish(pose_stamped_cov_msg);
}
